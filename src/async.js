...
function updateAsync(key, interval, saver, loader) {
    ...
    const asyncData = Memory.asyncData;
...
const energySources = function () {
    _.each(Game.rooms, function (room) {
        ...
        for (let source in sources) {
            ...
            if (Memory.rooms[room.name].sources[source.id] == undefined) {
                Memory.rooms[room.name].sources[source.id] = {}
            }
            const sourceMem = Memory.rooms[room.name].sources[source.id];
            ...
            let free_spots = 8;
            for (let direction in c.DIRECTIONS) {
                if (!c.DIRECTIONS.hasOwnProperty(direction)) continue;
                direction = c.DIRECTIONS[direction];
                const terrain = Game.map.getTerrainAt(source.pos.x + direction[0], source.pos.y + direction[1], room.name);
                if (terrain == "wall")
